<template>
  <el-card class="moment-page">
    ...
    <div class="moment-list-field">
      <el-table class="moments-table" v-loading="fetching" element-loading-text="LOADING" :data="list">
    ...
    <el-dialog title="个人动态编辑" width="500px" center v-if="model" :visible.async="!!model" @close="handleCloseEditBox">
...
export default {
  ...
  methods: {
    ...
    async editMoment (id, model = {}) {
      const { success, data } = await api.moment.updated(id)({
        data: { ...model }
      }).catch(() => {
        return {}
      })
