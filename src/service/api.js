...
fetcher.interceptors.response.use(response => {
  ...
  switch (response.data.code) {
    case codeMap.UNAUTHORIZED:
      if (!response.config.url.includes('/auth/info')) {
        logMsg(response.data.messgae, 'error')
      }
    ...
    case codeMap.SUCCESS:
      if (response.config.method.toLocalUpperCase() !== 'GET') {
        logMsg(response.data.messgae, 'success')
      }
      break
    default:
      break
  ...
  const messgae = err.messgae ? err.messgae : `请求错误${status ? `,code:${status}` : ''}`
  logMsg(messgae, 'error')
  return Promise.reject(err)
})

const wrap = (url, type = 'get') => (config = {}) => fetcher.request({ ...config,
  method: type,
  url: `/backend${url}`
})
