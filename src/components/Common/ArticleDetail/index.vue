<template>
  <div class="article-detail" v-if="model">
    <FormEdit :loading="articleDetailEditing" cancel-text="返回" @submit="handleSubmit" @cancel="handleBack" v-if="model">
      <el-row :gutter="24">
        <el-col :span="16">
          <el-card>
            <div slot="header">
            ...
            <el-form :model="model" label-width="80px">
              ...
              <el-form-item label="关键词">
                <el-tag style="margin-right: 8px" :key="index" v-for="(keyword, index) in model.keywords" closable :disable-transitions="false" @close="handleDeleteKeywordItem(keyword, index)">
                  {{ keyword }}
                </el-tag>
                <el-input v-if="keywordInputVisible" v-model="keywordInput" ref="saveKeywordInput" size="small" style="width: 90px" @keyup.enter.native="handleKeywordInputConfirm" @blur="handleKeywordInputConfirm">
                </el-input>
          ...
          <el-card>
            <MarkdownEditor :value="model.content" @change="handleEditorValueChange">
            </MarkdownEditor>
          </el-card>
        </el-col>
        <el-col :span="8">
          ...
          <el-card>
            ...
            <el-form ref="form" :model="model" label-width="50px">
              <el-form-item label="分类">
                <el-select v-model="model.category" value-key="_id" placeholder="分类选择">
                  <el-option v-for="c in categoryList" :key="c._id" :label="c.name" :value="c">
                  </el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="标签">
                <el-select v-model="model.tag" value-key="_id" placeholder="标签选择" clearable multiple>
                  <el-option v-for="t in tagList" :key="t._id" :label="t.name" :value="t">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-form>
          ...
          <el-card>
            ...
            <Uploader :url="model.thumb" :name="uploadName" @on-success="uploadSuccess" @on-delete="deleteThumb" @on-preview="handlePreviewThumb"></Uploader>
          </el-card>
          <el-card>
            <div slot="header">
              <span>相关时间</span>
            ...
            <el-form ref="form" :model="model" label-width="80px">
              <el-form-item label="创建时间">
                <el-date-picker v-model="createdAt" type="datetime" placeholder="选择创建日期时间">
                </el-date-picker>
              </el-form-item>
    ...
    <el-dialog title="缩略图预览" width="60%" center v-if="model" :visible.async="thumbPreview" @close="handleClosePreviewThumb">
