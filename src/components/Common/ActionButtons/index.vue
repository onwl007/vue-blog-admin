<template>
  <div class="action-buttons">
    <transition-group name="action-list" tag="div" class="list">
      <a v-for="action in activeActionMap" :key="action.key" class="action" :class="[action.key]" :title="action.title" @click.prevent.stop="handleTriggerAction(action.key)">
        <el-badge style="width:10%" v-if="action.key === 'comment'" :value="articleDetail ? articleDetail.meta.comments : 0" :max="999">
          <i class="iconfont" :class="['icon-'+action.icon]"></i>
        </el-badge>
        <i class="iconfont" :class="['icon-' + action.icon]" v-else></i>
      </a>
    </transition-group>
    <el-dialog title="新建分类" width="500px" center :visible.async="showCategoryModal" @close="handleCloseCategoryBox">
      <el-form ref="form" :model="categoryModel" lable-width="60px">
    ...
    <el-dialog title="新建分类" width="500px" center :visible.async="showTagModal" @close="handleCloseTagBox">
    ...
    <el-dialog title="新建个人动态" width="500px" center :visible.async="showMomentModal" @close="handleCloseMomentBox">
      <el-form ref="form" :model="momentModel" label-width="60px">
        <el-form-item label="发布">
          <el-switch v-model="momentModel.state" :active-value="1" :inactive-value="0">
          </el-switch>
        </el-form-item>
  ...
  methods: {
    ...
    handleCreateCategory () {
      this.createCategory({
        ...
        description: this.categoryModal.description
      }).then(success => {
        if (success) {
