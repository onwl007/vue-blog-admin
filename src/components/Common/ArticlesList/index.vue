<template>
  <div class="article-list-field">
    <el-table class="articles-table" v-loading="loading" element-loading-text="LOADING" :data="list" @sort-change="handleChangeSort">
      <el-table-column prop="thumb" width="120" label="缩略图">
      ...
      <el-table-column prop="meta.pvs" label="浏览量" width="80" sortable>
        <template slot-scope="scope">
          <span>{{ scope.row.meta.pvs }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="meta.ups" label="点赞数" width="80" sortable>
        <template slot-scope="scope">
          <span>{{ scope.row.meta.ups }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="meta.comments" label="评论量" width="80" sortable>
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button class="operate" :class="[scope.row.state == 1 ? 'unpublish-btn' : 'publish-btn']" type="text" @click="handleTogglePublish(scope.$index, scope.row, !~~scope.row.state)">
            <i class="iconfont" :class="[`icon-${scope.row.state == 1 ? 'unpublish' : 'publish'}`]"></i>
          </el-button>
          <el-button class="operate edit-btn" type="text" @click="handleEdit(scope.$index, scope.row)">
            <i class="iconfont icon-edit"></i>
          </el-button>
          <el-button class="operate delete-btn" type="text" @click="handleOpenDeleteBox(scope.$index, scope.row)">
            <i class="iconfont icon-delete"></i>
          </el-button>
          <a class="el-button el-button--text operate qrcode-btn" :class="{ 'is-disabled': scope.row.state !== 1 }" @click="handleViewQrcode(scope.$index, scope.row)">
            <i class="iconfont icon-qrcode"></i>
          </a>
          <a :href="getArticleLink(scope.row)" target="_blank" class="el-button el-button--text operate skip-btn" :class="{ 'is-disabled': scope.row.state !== 1 }">
            <i class="iconfont icon-skip-link"></i>
          </a>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination class="pagination" @current-change="handlePageChange" :current-page.sync="pagination.current_page" :page-size="pagination.per_page" layout="total, prev, pager, next, jumper" :total="pagination.total">
    </el-pagination>
    <el-dialog title="文章二维码" width="400px" center :visible.async="!!qrcodeUrl" @close="handleCloseQrcodeThumb">
...
export default {
  ...
  methods: {
    ...
    getArticleLink (data) {
      ...
      return data.permalink || `https://onwl007.me/blog/article/${data._id}`
    },
...
.articles-table {
  ...
  .state {
    ...
    &.published {
      i {
        background-color: alpha($green, 0.4);
